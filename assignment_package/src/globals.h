...
{
    // Heuristic functions should return 0 if their denominators would be 0.
...
{
    //  sHeuristic functions should return 0 if their denominators would be 0.
    if (fPdf == 0.f && gPdf == 0.f) return 0.f;
    // hard code beta = 2
    Float f = nf * fPdf;
    Float g = ng * gPdf;
...
inline float maxComponent(Color3f beta)
{
    int max = ( beta[0] < beta[1] ) ? beta[1] : beta[0];
    return ( ( max < beta[2] ) ? beta[2] : max );
}
