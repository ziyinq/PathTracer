...
bool Scene::IntersectP(const Ray &ray) const
{
    for (std::shared_ptr<Primitive> p : primitives)
    {
        // if this primitive is not light
...
bool Scene::IntersectTr(Ray ray, std::shared_ptr<Sampler> sampler, Intersection *isect, Color3f *Tr) const
{
    *Tr = Color3f(1.f);
    while (true){
        ...
        // TODO: modify intersect and ray to get tmax?
        if (ray.medium)
...
void Scene::CreateTestScene()
{
    ...
    auto lightSource = std::make_shared<DiffuseAreaLight>(lightSquare->transform, Color3f(1,1,1), lightSquare);
    ...
    auto lightSource2 = std::make_shared<DiffuseAreaLight>(lightSquare2->transform, Color3f(0.9,1,0.7), lightSquare2, true);
